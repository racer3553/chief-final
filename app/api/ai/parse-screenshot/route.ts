...
const PROMPTS: Record<string, string> = {
  ...
  moza: `This is a screenshot of MOZA Pit House (wheel base + pedals config). Read EVERY visible value and return strict JSON with these keys (set null for anything not visible): max_force_nm, ffb_strength_pct, road_feel_pct, wheel_inertia_pct, damping_pct, friction_pct, return_force_pct, mechanical_damper_pct, ffb_reverse, soft_lock_pct, steering_range_deg, throttle_min_pct, throttle_max_pct, throttle_curve, brake_min_pct, brake_max_pct, brake_force_kg, brake_curve, clutch_min_pct, clutch_max_pct, clutch_curve, ffb_smoothing, ffb_filter, current_profile, firmware_version. Output ONLY the JSON.`,
  fanatec: `This is a screenshot of Fanalab or Fanatec Control Panel. Read EVERY visible FFB / wheel setting (SEN, FF, SHO, BLI, DRI, FOR, SPR, DPR, NDP, NFR, INT, FEI, NIN, brake force %, throttle min/max, clutch settings, gear shifter mode). Return strict JSON with snake_case keys. Output ONLY the JSON.`,
  thrustmaster: `This is a screenshot of Thrustmaster Advanced Control Panel. Read EVERY visible value (overall FFB strength, constant/periodic/spring/damper effects, max degree rotation, pedal calibration, brake pressure). Return strict JSON. Output ONLY the JSON.`,
  asetek: `This is a screenshot of Asetek SimSports RaceHub (Invicta / Forte / La Prima). Read EVERY visible wheel + pedal value including FFB strength, damping, friction, motor max torque, brake pressure curve, throttle calibration. Return strict JSON. Output ONLY the JSON.`,
